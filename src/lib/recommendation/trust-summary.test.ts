...
describe("buildRecommendationTrustSummary", () => {
  it("uses buyer-facing copy for the current recommendation set", () => {
    ...
    expect(summary.headline).not.toMatch(/проверен/iu);
    expect(summary.reviewMessage).not.toMatch(/ручн|перепровер/iu);
  });
});
