...
export function resolveRidingStylesTruth(value, context = {}) {
  ...
  if (
    /all[\s-]*mountain/u.test(text) ||
    /универсальн/u.test(text) ||
    /(?:^|[^\p{L}\p{N}_])олмаунтин(?=$|[^\p{L}\p{N}_])/u.test(text)
  ) {
...
export function resolveCamberTruth(value, context = {}, correctedValue = null) {
  if (correctedValue != null) {
    if (!CAMBERS.has(correctedValue)) return unknown(context, "unrecognized");
    return knownTruth(correctedValue, context, {
      provenance: "manual",
  ...
  if (CAMBERS.has(candidate)) {
    return knownTruth(candidate, context, { method: "explicit" });
  }
  if (candidate === "кэмбер") {
    return knownTruth("camber", context, {
      method: "normalized",
