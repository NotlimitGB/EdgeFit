...
describe("attribute truth resolvers", () => {
  it.each([
    ...
    ["олмаунтин", ["all-mountain"], "known"],
    ["  ОЛМАУНТИН  ", ["all-mountain"], "known"],
    ["(олмаунтин)", ["all-mountain"], "known"],
    ["олмаунтин / фристайл", ["all-mountain", "park"], "known"],
    ["олмаунтин фристайл", ["all-mountain", "park"], "known"],
    ["олмаунтин, фрирайд", ["all-mountain", "freeride"], "known"],
  ...
  it.each(["олмаунт", "олмаунтиновый", "xолмаунтин", "олмаунтинx"])(
    "does not broaden the confirmed олмаунтин vocabulary for %j",
    (source) => {
      expect(resolveRidingStylesTruth(source, context)).toMatchObject({
        value: null,
        evidence: { state: "unknown", method: null },
      });
    },
  );
  ...
  it.each(["camber", "rocker", "flat", "hybrid-camber", "hybrid-rocker"])(
    "keeps canonical camber profile %j explicit",
    (source) => {
      expect(resolveCamberTruth(source, context)).toEqual({
        value: source,
        evidence: {
          ...context,
          state: "known",
  ...
  it.each(["Кэмбер", "  КЭМБЕР  "])(
    "normalizes confirmed Russian camber evidence from %j",
    (source) => {
      expect(resolveCamberTruth(source, context)).toEqual({
        value: "camber",
        evidence: {
  ...
  it.each(["Гибрид", "гибридный", "nonsense"])(
    "keeps unconfirmed camber vocabulary %j unknown",
    (source) => {
      expect(resolveCamberTruth(source, context)).toEqual({
        value: null,
        reason: "unrecognized",
  ...
  it("keeps an authorized camber correction manual despite merchant conflict", () => {
    expect(resolveCamberTruth("Гибрид", context, "hybrid-camber")).toEqual({
      value: "hybrid-camber",
      evidence: {
        ...context,
        state: "known",
